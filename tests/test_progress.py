...
def test_initialization(temp_tracker):
    ...
    assert "LLM Building Plan (Days 1‑30)" in progress
    assert "Runtime Layer (Days 31‑50)" in progress
    assert progress["LLM Building Plan (Days 1‑30)"] == 0


def test_update_phase(temp_tracker):
    """Test updating a phase's progress."""
    temp_tracker.update_phase("LLM Building Plan (Days 1‑30)", 5)
    progress = temp_tracker.get_progress()
    assert progress["LLM Building Plan (Days 1‑30)"] == 5


def test_get_total(temp_tracker):
    """Test getting total completed days."""
    temp_tracker.update_phase("LLM Building Plan (Days 1‑30)", 5)
    temp_tracker.update_phase("Runtime Layer (Days 31‑50)", 2)
